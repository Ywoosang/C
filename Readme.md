...
주의 : `int a[101];` 처럼 공간을 할당한 후 요소를 삽입  
그 다음, 요소가 들어가있는 배열의 크기를 구하고 싶어 `sizeof(a)/sizeof(int)` 를 이용하면 원하는 결과가 나오지 않음  
`sizeof(a)/sizeof(int)` 는 공간을 할당하지 않은 요소까지 배열의 크기로 함께 카운트하므로 주의  
...
### 정수를 문자열 배열로 변경 sprintf

정수 123 을 문자열 "123" 으로 변경하고 싶을 때  
...
다음과 같은 문자열 배열이 있을 때 b 문자열 값을 복사해 a 에 대입  
```c
strcpy(복사받을 변수,복사할 변수 또는 값);
``` 
ex) 
```c
int i;
...
주의 : 문자열은 대입연산자로 대입 불가  `a = b (x)`  

### 구조체

기본형 데이터를 하나로 묶은 데이터 구조  
...
문자열 대입
```c
strcpy(구조체변수명.변수이름,문자열값);
```
값 출력
```c
printf("%자료형",구조체변수명.변수이름)
```
값 받아서 대입
```c
scanf("%s",구조체변수명.변수이름)
...
변수는 컴퓨터의 메모리에 생성  
메모리에 일정한 공간을 확보해두고 원하는 값을 저장하거나 가져옴  
변수는 `a` 같이 이름으로 사용하지만 메모리의 특정 장소에 위치하고 있으므로 메모리 주소로도 표현 가능  

`&` : 메모리 주소  
메모리 주소는 008AF7FC과 같이 16진수 형태이며 printf에서 서식 지정자 %p를 사용하여 출력  
변수 a 의 메모리 실제 주소를 출력  
```c 
printf("%p",&a);
```
ex) 0x7ffcdeaa2e74
...
포인터는 변수의 주소만 가리킴  
역참조는 주소에 있는 값을 가져옴    
a 의 메모리 주소에 있는 값을 출력  
```c
printf("%d",*(&a));
```
포인터 선언  
  
정수형 변수의 주소를 저장할 수 있는 포인터 변수 선언  
```c
int *pa; 
``` 
포인터 변수에 메모리 주소 할당  
...
### 참조하고 있는 메모리 주소 대상 변수의 값 변경  

포인터변수를 선언하고 메모리 주소 할당
```c
int a=10,b=20;
int *pa;  
pa = &a;  
```
포인터 변수가 가리키고 있는 메모리의 값에 b 값 할당  
즉, a 에 b 값인 20을 할당  
```c
*pa = b;  
```
포인터 변수가 가리키고 있는 메모리의 값에 1 더함  
결과적으로 a도 21 로 변함 
```c
*pa = *pa +1; 
``` 
### 값에 의한 호출 (Call by valye)

a,b 값을 파라미터로 전달  
함수에서 pa,pb 로 받은값을 스왑   
이때 pa,pb 는 새로운 메모리 주소가 생성 후 전달받은 a,b 값이 들어가있는 상태  
pa,pb의 스왑으로 인해 a,b 의 값은 변하지 않음  
...
a,b 의 주소를 파라미터인 포인터 변수에 전달  
메모리 주소에 해당하는 변수의 값을 변경하므로 포인터가 가리키는 변수의 변경  
즉 a,b 값이 변경됨    
```c
void swap(int *pa,int *pb){
    int tmp;
